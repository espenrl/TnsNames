...
namespace erl.Oracle.TnsNames.Antlr4.Runtime.Dfa
{
        ...
        where T : class
    {
        ...
        public override AbstractEdgeMap<T> PutAll(IEdgeMap<T> m)
        {
            ...
            if (m is erl.Oracle.TnsNames.Antlr4.Runtime.Dfa.ArrayEdgeMap<T>)
            {
                erl.Oracle.TnsNames.Antlr4.Runtime.Dfa.ArrayEdgeMap<T> other = (erl.Oracle.TnsNames.Antlr4.Runtime.Dfa.ArrayEdgeMap<T>)m;
                int minOverlap = Math.Max(minIndex, other.minIndex);
                int maxOverlap = Math.Min(maxIndex, other.maxIndex);
                erl.Oracle.TnsNames.Antlr4.Runtime.Dfa.ArrayEdgeMap<T> result = this;
                for (int i = minOverlap; i <= maxOverlap; i++)
                {
                    result = ((erl.Oracle.TnsNames.Antlr4.Runtime.Dfa.ArrayEdgeMap<T>)result.Put(i, m[i]));
                }
                return result;
            }
            else
                ...
                else
                {
                    if (m is SparseEdgeMap<T>)
                    {
                        ...
                        lock (other)
                        {
                            ...
                            {
                                result = ((erl.Oracle.TnsNames.Antlr4.Runtime.Dfa.ArrayEdgeMap<T>)result.Put(keys[i], values[i]));
                            }
                            return result;
                        }
                    }
