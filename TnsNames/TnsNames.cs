...
namespace erl.Oracle.TnsNames
{
    ...
    public static class TnsNames
    {
        ...
        {
            if (tnsNamesFileInfo == null) throw new ArgumentNullException("tnsNamesFileInfo");

            var inputStream = new AntlrFileStream(tnsNamesFileInfo.Filepath);
        ...
        private static IDatabaseAddress CreateDatabaseAddress(DatabaseAddressNode address)
        {
            ...
            if (string.Equals("TCP", protocol, StringComparison.InvariantCulture)
                || string.Equals("TCPS", protocol, StringComparison.InvariantCulture)
                || string.Equals("SDP", protocol, StringComparison.InvariantCulture))
            {
                return new IpDatabaseAddress(protocol, address.Host, address.PortNumber, rawAddress);
            }
            if (string.Equals("IPC", protocol, StringComparison.InvariantCulture))
            {
                return new IpcDatabaseAddress(protocol, address.Key, rawAddress);
            }
            if (string.Equals("NMP", protocol, StringComparison.InvariantCulture))
            {
