...
namespace erl.Oracle.TnsNames.ANTLR
{
    public class TnsNamesListener : TnsNamesParserBaseListener
    {
        ...
        public List<TnsNamesListenerErrorNode> ErrorNodes { get; private set; }
        public List<TnsNameNode> TnsItems { get; private set; }
        public List<string> IfileEntries { get; private set; }
        ...
        public override void ExitParameter(TnsNamesParser.ParameterContext context)
        {
            ...
            if (isRootLevel && !string.Equals("IFILE", keyword, StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                foreach (var databaseAddress in _currentTnsNameAddressList)
                {
                    ...
                    if (string.Equals("TCP", databaseAddress.Protocol, StringComparison.InvariantCulture)
                        || string.Equals("TCPS", databaseAddress.Protocol, StringComparison.InvariantCulture)
                        || string.Equals("SDP", databaseAddress.Protocol, StringComparison.InvariantCulture))
                    {
                        // Host
                    ...
                    else if (string.Equals("IPC", databaseAddress.Protocol, StringComparison.InvariantCulture))
                    {
                        // Key
                    ...
                    else if (string.Equals("NMP", databaseAddress.Protocol, StringComparison.InvariantCulture))
                    {
                        // Server
        ...
        public override void EnterScalarParameterValue(TnsNamesParser.ScalarParameterValueContext context)
        {
            ...
            if (isAddressLevel && string.Equals(keyword, "HOST", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentDatabaseAddress.Host = text.ToUpperInvariant();
            }
            else if (isAddressLevel && string.Equals(keyword, "KEY", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentDatabaseAddress.Key = text.ToUpperInvariant();
            }
            else if (isAddressLevel && string.Equals(keyword, "PIPE", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentDatabaseAddress.Pipe = text.ToUpperInvariant();
            }
            else if (isAddressLevel && string.Equals(keyword, "PORT", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentDatabaseAddress.PortStr = text;
            }
            else if (isAddressLevel && string.Equals(keyword, "PROTOCOL", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentDatabaseAddress.Protocol = text.ToUpperInvariant();
            }
            else if (isAddressLevel && string.Equals(keyword, "SERVER", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentDatabaseAddress.Server = text.ToUpperInvariant();
            }
            else if (isConnectDataLevel && string.Equals(keyword, "SERVICE_NAME", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentTnsNameServiceName = text.ToUpperInvariant();
            }
            else if (isConnectDataLevel && string.Equals(keyword, "SID", StringComparison.InvariantCultureIgnoreCase))
            {
                _currentTnsNameSid = text.ToUpperInvariant();
            }
            else if (isRootLevel && string.Equals(keyword, "IFILE", StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        public override void EnterParameterListParameterValue(TnsNamesParser.ParameterListParameterValueContext context)
        {
            ...
            // TNS name node
            if (isRootLevel && !string.Equals(keyword, "IFILE", StringComparison.InvariantCultureIgnoreCase))
            {
            ...
            // Address node
            else if (string.Equals(keyword, "ADDRESS", StringComparison.InvariantCultureIgnoreCase))
            {
